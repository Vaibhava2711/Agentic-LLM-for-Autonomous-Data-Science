...
class ReactCompatMixin:
    ...
    @staticmethod
    def _split_action_action_input(
        response: str, keyword: AgentKeyword
    ) -> List["Function"]:
    ...
    def get_toolcall(self, response: str) -> List["Function"]:
        ...
        if len(functions) == 0 and self.keyword != ReactCompatMixin.keyword:
            # compat react
            functions = self._split_action_action_input(
                response, ReactCompatMixin.keyword
            )
        return functions
    ...
    def _format_tool_calls(self, tool_call_messages) -> str:
        ...
        for message in tool_call_messages:
            ...
            tool_calls.append(
                f'{self.keyword.action} {tool_call["name"]}\n'
...
class BaseAgentTemplate(ReactCompatMixin, ABC):
    ...
    def _parse_tool(tool, lang: Literal["zh", "en"]) -> ToolDesc:
        ...
        assert (
            name_for_model is not None and description is not None
        ), f"tool_desc: {tool_desc}"
        return tool_desc
