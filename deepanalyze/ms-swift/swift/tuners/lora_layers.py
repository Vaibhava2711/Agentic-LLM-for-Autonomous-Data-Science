        ...
        for layer_name in self.adapter_layer_names:
            ...
            for key, layer in module_dict.items():
                if key == adapter_name:
                    if activate:
                        SwiftAdapter.save_memory(
                            layer, layer_name + "." + key, self.module_key, True
                        )
                    else:
                        SwiftAdapter.save_memory(
                            layer,
...
if is_bnb_available():
    ...
    def dispatch_bnb_8bit(
        target: torch.nn.Module, adapter_name: str, module_key: str, **kwargs
    ):
        new_module = None
        ...
        **optional_kwargs,
        ...
        if isinstance(target, LoraLayer) and not isinstance(target, AdaLoraLayer):
            ...
            new_module = self._create_new_module(
                lora_config, adapter_name, target, current_key=current_key, **kwargs
            )
...
class MergedLinear(nn.Linear, LoRALayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            base_layer.out_features % len(enable_lora) == 0
        ...
        if r > 0 and any(enable_lora):
            self.lora_A = nn.Parameter(
                self.weight.new_zeros((r * sum(enable_lora), base_layer.in_features))
            )
            self.lora_B = nn.Parameter(
                self.weight.new_zeros(
                    (base_layer.out_features // len(enable_lora) * sum(enable_lora), r)
                )
            ...
            self.lora_ind = self.weight.new_zeros(
                (base_layer.out_features,), dtype=torch.bool
...
def mark_lora_as_trainable(
    model: nn.Module, adapter_name: str, bias: str = "none"
) -> None:
    if bias == "none":
        return
    elif bias == "all":
...
def lora_state_dict(
    state_dict, adapter_name: str, bias: str = "none"
