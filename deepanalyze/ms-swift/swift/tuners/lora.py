...
class LoRAConfig(LoraConfig, SwiftConfig):
    ...
    use_qa_lora: bool = field(
        default=False,
        metadata={
            "help": "Use [qa-lora](https://github.com/yuhuixu1993/qa-lora) or not"
        },
    )
    ...
    enable_lora: List[bool] = field(
        default=None,
        metadata={
            "help": "The modules need to be turned on when using the merged linear layer"
        },
    )
    ...
    lora_dtype: Optional[str] = field(
        default=None,
        metadata={
            "help": "The lora dtype, default None means following the original layer's dtype"
        },
    )
    ...
    def prepare_model(model: nn.Module, config: LoRAConfig, adapter_name: str):
        ...
        def state_dict_callback(state_dict, adapter_name, cfg=None, **kwargs):
            return lora_state_dict(
                state_dict, adapter_name, cfg.bias if cfg else config.bias
            )

        def mark_trainable_callback(model, cfg=None):
            mark_lora_as_trainable(
                model, adapter_name, cfg.bias if cfg else config.bias
            )
