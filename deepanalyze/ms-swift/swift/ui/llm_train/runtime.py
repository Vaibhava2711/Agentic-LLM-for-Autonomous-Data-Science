...
class Runtime(BaseUI):
    ...
    def do_build_ui(cls, base_tab: Type['BaseUI']):
        with gr.Accordion(elem_id='runtime_tab', open=False):
            with gr.Blocks():
                ...
                base_tab.element('show_log').click(Runtime.update_log, [base_tab.element('running_tasks')], [cls.element('log')] + cls.all_plots).then(Runtime.wait, [base_tab.element('logging_dir'), base_tab.element('running_tasks')], [cls.element('log')] + cls.all_plots, **concurrency_limit)
                base_tab.element('stop_show_log').click(cls.break_log_event, [cls.element('running_tasks')], [])
                base_tab.element('start_tb').click(Runtime.start_tb, [base_tab.element('logging_dir')], [base_tab.element('tb_url')])
                base_tab.element('close_tb').click(Runtime.close_tb, [base_tab.element('logging_dir')], [])
                base_tab.element('refresh_tasks').click(partial(Runtime.refresh_tasks, group=cls.group), [base_tab.element('running_tasks')], [base_tab.element('running_tasks')])

    @classmethod
    def after_build_ui(cls, base_tab: Type['BaseUI']):
        cls.element('show_running_cmd').click(Runtime.show_train_sh, cls.element('running_cmd'), [cls.element('show_sh')] + [cls.element('cmd_sh')])
    ...
    def refresh_tasks(running_task=None, group=None):
        ...
        for proc in psutil.process_iter():
            try:
                ...
                cmdlines = []
            if any([process_name in cmdline for cmdline in cmdlines]) and (not any([negative_name in cmdline for negative_name in negative_names for cmdline in cmdlines])) and any([cmdline in cmd_name for cmdline in cmdlines]):
    ...
    def plot(task):
        ...
        if not os.path.exists(tb_dir):
            return [None] * len(plot)
        fname = [fname for fname in os.listdir(tb_dir) if os.path.isfile(os.path.join(tb_dir, fname)) and fname.startswith('events.out')]
        if fname:
            fname = fname[0]
        else:
            return [None] * len(plot)
