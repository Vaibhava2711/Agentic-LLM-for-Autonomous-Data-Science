...
class LLMTrain(BaseUI):
    ...
    def do_build_ui(cls, base_tab: Type['BaseUI']):
        with gr.TabItem(elem_id='llm_train', label=''):
            ...
            with gr.Blocks():
                ...
                cls.element('train_type').change(Hyper.update_lr, inputs=[base_tab.element('train_type')], outputs=[cls.element('learning_rate')])
                submit.click(cls.train_local, list(cls.valid_elements().values()), [cls.element('running_cmd'), cls.element('logging_dir'), cls.element('runtime_tab'), cls.element('running_tasks'), cls.element('train_record')])
                ...
                base_tab.element('running_tasks').change(partial(Runtime.task_changed, base_tab=base_tab), [base_tab.element('running_tasks')], list(base_tab.valid_elements().values()) + [cls.element('log')] + Runtime.all_plots)
                Runtime.element('kill_task').click(Runtime.kill_task, [Runtime.element('running_tasks')], [Runtime.element('running_tasks')] + [Runtime.element('log')] + Runtime.all_plots).then(Runtime.reset, [], [Runtime.element('logging_dir')] + [Hyper.element('output_dir')])
    ...
    def train(cls, *args):
        ...
        for (key, value) in zip(keys, args):
            ...
            if key not in ignore_elements and key in default_args and (compare_value != value) and (value or value in (0, False)):
        ...
        try:
            sft_args = RLHFArguments(**{key: value.split(' ') if kwargs_is_list.get(key, False) and isinstance(value, str) else value for (key, value) in kwargs.items()})
        ...
        params += f'--add_version False --output_dir {sft_args.output_dir} --logging_dir {sft_args.logging_dir} --ignore_args_error True'
        ddp_param = ''
    ...
    def train_studio(cls, *args):
        ...
        if not other_kwargs['dry_run']:
            ...
            yield (['Current is dryrun mode so you can only view the training cmd, please duplicate this space to do training or use with inference.'] + [None] * len(Runtime.sft_plot) + [run_command])
    ...
    def train_local(cls, *args):
        ...
        return (run_command, sft_args.logging_dir, gr.update(open=True), Runtime.refresh_tasks(sft_args.output_dir, cls.group), gr.update(choices=cls.list_cache(sft_args.model)))

    @classmethod
    def prepare_sub_to_filter(cls):
        tabs_relation_dict = {key: val for (key, val) in zip(['train_type', 'optimizer', 'task_type'], [Tuner.tabs_to_filter, Optimizer.tabs_to_filter, Task.tabs_to_filter])}
        ...
        for (target, tabs_to_filter) in tabs_relation_dict.items():
            ...
            for tab_key in tabs_to_filter.keys():
                ...
                if tab_key == 'lisa' and target_value == 'full' and uncleaned_kwargs.get('lisa_activated_layers'):
                    continue
                if tab_key == 'lora_ga' and target_value == 'lora' and (uncleaned_kwargs.get('init_weights') == 'lora-ga'):
                    continue
