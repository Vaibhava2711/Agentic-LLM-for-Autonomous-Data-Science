...
class LLMRollout(BaseUI):
    ...
    locale_dict = {'tensor_parallel_size': {'label': {'zh': 'Tensor parallel size', 'en': 'Tensor parallel size'}}, 'data_parallel_size': {'label': {'zh': 'Data parallel size', 'en': 'Data parallel size'}}, 'max_model_len': {'label': {'zh': 'Max model len', 'en': 'Max model len'}}, 'gpu_memory_utilization': {'label': {'zh': 'GPU memory utilization', 'en': 'GPU memory utilization'}}, 'port': {'label': {'zh': 'Rollout Port', 'en': 'Rollout Port'}}, 'llm_rollout': {'label': {'zh': 'External rollout model deployment', 'en': 'External rollout model deployment'}}, 'rollout': {'value': {'zh': 'Start Rollout', 'en': 'Start Rollout'}}, 'load_alert': {'value': {'zh': 'Start to rollout, please Click "Show running status" to view details', 'en': 'Start to rollout, please Click "Show running status" to view details'}}, 'port_alert': {'value': {'zh': 'The port has been occupied', 'en': 'The port has been occupied'}}, 'rollout_gpu_id': {'label': {'zh': 'Choose GPU for rollout', 'en': 'Choose GPU for rollout'}}, 'more_roll_params': {'label': {'zh': 'More rollout params', 'en': 'More rollout params'}, 'info': {'zh': 'Fill in with json format or --xxx xxx cmd format', 'en': 'Fill in with json format or --xxx xxx cmd format'}}}
        ...
        with gr.Accordion(elem_id='llm_rollout', open=False, visible=False):
            ...
            with gr.Blocks():
                ...
                with gr.Row(equal_height=True):
                    gr.Dropdown(elem_id='rollout_gpu_id', multiselect=True, choices=[str(i) for i in range(device_count)] + ['cpu'], value=default_device, scale=4)
                ...
                base_tab.element('rollout_running_tasks').change(partial(RolloutRuntime.task_changed, base_tab=base_tab), [base_tab.element('rollout_running_tasks')], list(cls.valid_elements().values()) + [cls.element('rollout_log')])
                RolloutRuntime.element('rollout_kill_task').click(RolloutRuntime.kill_task, [RolloutRuntime.element('rollout_running_tasks')], [RolloutRuntime.element('rollout_running_tasks')] + [RolloutRuntime.element('rollout_log')])
    ...
    def rollout(cls, *args):
        ...
        rollout_args = RolloutArguments(**{key: value.split(' ') if key in kwargs_is_list and kwargs_is_list[key] else value for (key, value) in kwargs.items()})
