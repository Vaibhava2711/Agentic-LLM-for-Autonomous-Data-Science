...
class LLMInfer(BaseUI):
    ...
    locale_dict = {'generate_alert': {'value': {'zh': 'Please deploy model first', 'en': 'Please deploy model first'}}, 'port': {'label': {'zh': 'Port', 'en': 'Port'}}, 'llm_infer': {'label': {'zh': 'LLM Inference', 'en': 'LLM Inference'}}, 'load_alert': {'value': {'zh': 'Start to deploy model, please Click "Show running status" to view details', 'en': 'Start to deploy model, please Click "Show running status" to view details'}}, 'loaded_alert': {'value': {'zh': 'Model loaded', 'en': 'Model loaded'}}, 'port_alert': {'value': {'zh': 'The port has been occupied', 'en': 'The port has been occupied'}}, 'chatbot': {'value': {'zh': 'Chat bot', 'en': 'Chat bot'}}, 'infer_model_type': {'label': {'zh': 'LoRA module', 'en': 'LoRA module'}, 'info': {'zh': 'Which LoRA to use on server, default value is `default`', 'en': 'Which LoRA to use on server, default value is `default`'}}, 'prompt': {'label': {'zh': 'Input:', 'en': 'Input:'}}, 'clear_history': {'value': {'zh': 'Clear history', 'en': 'Clear history'}}, 'submit': {'value': {'zh': '🚀 Send', 'en': '🚀 Send'}}, 'gpu_id': {'label': {'zh': 'Choose GPU', 'en': 'Choose GPU'}, 'info': {'zh': 'Select GPU to train', 'en': 'Select GPU to train'}}}
    ...
    def do_build_ui(cls, base_tab: Type['BaseUI']):
        with gr.TabItem(elem_id='llm_infer', label=''):
            ...
            with gr.Blocks():
                ...
                cls.element('load_checkpoint').click(cls.deploy_model, list(base_tab.valid_elements().values()), [cls.element('runtime_tab'), cls.element('running_tasks')])
                submit.click(cls.send_message, inputs=[cls.element('running_tasks'), cls.element('template'), prompt, image, video, audio, infer_request, infer_model_type, cls.element('system'), cls.element('max_new_tokens'), cls.element('temperature'), cls.element('top_k'), cls.element('top_p'), cls.element('repetition_penalty')], outputs=[prompt, chatbot, image, video, audio, infer_request], queue=True)
                clear_history.click(fn=cls.clear_session, inputs=[], outputs=[prompt, chatbot, image, video, audio, infer_request])
                base_tab.element('running_tasks').change(partial(Runtime.task_changed, base_tab=base_tab), [base_tab.element('running_tasks')], list(cls.valid_elements().values()) + [cls.element('log')])
                Runtime.element('kill_task').click(Runtime.kill_task, [Runtime.element('running_tasks')], [Runtime.element('running_tasks')] + [Runtime.element('log')])
    ...
    def deploy(cls, *args):
        ...
        deploy_args = DeployArguments(**{key: value.split(' ') if key in kwargs_is_list and kwargs_is_list[key] else value for (key, value) in kwargs.items()})
    ...
    @classmethod
    def send_message(cls, running_task, template_type, prompt: str, image, video, audio, infer_request: InferRequest, infer_model_type, system, max_new_tokens, temperature, top_k, top_p, repetition_penalty):
        ...
        request_config = RequestConfig(temperature=temperature, top_k=top_k, top_p=top_p, repetition_penalty=repetition_penalty)
        ...
        gen_list = InferClient(port=args['port']).infer(infer_requests=[_infer_request], request_config=request_config, **model_kwargs)
