    ...
    def do_build_ui(cls, base_tab: Type['BaseUI']):
        with gr.Accordion(elem_id='runtime_tab', open=False, visible=True):
            with gr.Blocks():
                ...
                base_tab.element('show_log').click(cls.update_log, [], [cls.element('log')]).then(cls.wait, [base_tab.element('running_tasks')], [cls.element('log')], **concurrency_limit)
    ...
    def refresh_tasks(cls, running_task=None):
        ...
        for proc in psutil.process_iter():
            try:
                ...
                cmdlines = []
            if any([process_name in cmdline for cmdline in cmdlines]) and (not any([negative_name in cmdline for cmdline in cmdlines])) and any([cmd_name == cmdline for cmdline in cmdlines]):
                process.append(cls.construct_running_task(proc))
                if log_file is not None and any([log_file == cmdline for cmdline in cmdlines]):
