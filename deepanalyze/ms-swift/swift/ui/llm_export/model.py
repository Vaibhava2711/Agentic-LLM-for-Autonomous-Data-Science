...
class Model(BaseUI):
    ...
    def do_build_ui(cls, base_tab: Type['BaseUI']):
        with gr.Row():
            all_models = [model for model in get_all_models() if not any([ignored in model for ignored in cls.ignored_models])]
            gr.Dropdown(elem_id='model', scale=20, choices=all_models, value='Qwen/Qwen2.5-7B-Instruct', allow_custom_value=True)
    ...
    def after_build_ui(cls, base_tab: Type['BaseUI']):
        cls.element('model').change(partial(cls.update_input_model, arg_cls=ExportArguments, has_record=False), inputs=[cls.element('model')], outputs=list(cls.valid_elements().values()))
