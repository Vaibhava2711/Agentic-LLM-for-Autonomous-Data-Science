...
class LLMExport(BaseUI):
    ...
    locale_dict = {'llm_export': {'label': {'zh': 'LLM Export', 'en': 'LLM Export'}}, 'more_params': {'label': {'zh': 'More params', 'en': 'More params'}, 'info': {'zh': 'Fill in with json format or --xxx xxx cmd format', 'en': 'Fill in with json format or --xxx xxx cmd format'}}, 'export': {'value': {'zh': 'Begin Export', 'en': 'Begin Export'}}, 'gpu_id': {'label': {'zh': 'Choose GPU', 'en': 'Choose GPU'}, 'info': {'zh': 'Select GPU to export', 'en': 'Select GPU to export'}}}
    ...
    def do_build_ui(cls, base_tab: Type['BaseUI']):
        with gr.TabItem(elem_id='llm_export', label=''):
            ...
            with gr.Blocks():
                ...
                cls.element('export').click(cls.export_model, list(base_tab.valid_elements().values()), [cls.element('runtime_tab'), cls.element('running_tasks')])
                base_tab.element('running_tasks').change(partial(ExportRuntime.task_changed, base_tab=base_tab), [base_tab.element('running_tasks')], list(base_tab.valid_elements().values()) + [cls.element('log')])
                ExportRuntime.element('kill_task').click(ExportRuntime.kill_task, [ExportRuntime.element('running_tasks')], [ExportRuntime.element('running_tasks')] + [ExportRuntime.element('log')])
    ...
    def export(cls, *args):
        ...
        export_args = ExportArguments(**{key: value.split(' ') if key in kwargs_is_list and kwargs_is_list[key] else value for (key, value) in kwargs.items()})
        params = ''
