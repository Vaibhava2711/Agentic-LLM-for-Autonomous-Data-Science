...
class TestRun(unittest.TestCase):
    ...
    def test_template(self):
        ...
        result = infer_main(
            InferArguments(
                model=last_model_checkpoint, load_data_args=True, val_dataset_sample=2
            )
        )
        ...
        for quant_bits in quant_bits_list:
            ...
            if __name__ == "__main__":
                infer_args = InferArguments(
                    ...
                    merge_lora={0: True, 4: False}[quant_bits],
    ...
    def test_vl_audio(self):
        ...
        dataset_list = [
            ...
            "speech_asr/speech_asr_aishell1_trainsets:validation#100",
        ]
        for model, dataset in zip(model_type_list, dataset_list):
            ...
            infer_args = InferArguments(
                adapters=best_model_checkpoint,
                load_data_args=True,
                stream={"Qwen/Qwen-VL-Chat": True, "Qwen/Qwen-Audio-Chat": False}[
                    model
                ],
                val_dataset_sample=5,
            )
    ...
    def test_custom_dataset(self):
        ...
        for num_train_epochs in [1, 2]:
            sft_args = TrainArguments(
                ...
                dataset=["swift/self-cognition#20"]
                + [os.path.join(folder, fname) for fname in train_dataset_fnames],
                val_dataset=[
    ...
    def test_rlhf(self):
        ...
        for rlhf_type in rlhf_types:
            dataset = (
                "AI-ModelScope/hh_rlhf_cn:harmless_base_cn#100"
                if rlhf_type != "kto"
                else "AI-ModelScope/ultrafeedback-binarized-preferences-cleaned-kto#100"
            )
            train_kwargs = {}
            if rlhf_type == "ppo":
        ...
        for use_swift_lora in [False, True]:
            ...
            output = sft_main(
                [
                    ...
                    "5",
                    "--dataset",
                    "AI-ModelScope/leetcode-solutions-python#200",
                    "--output_dir",
                    output_dir,
...
class TestTrainer(unittest.TestCase):
    ...
    def test_trainer(self):
        ...
        trainer = BertTrainer(
            model, trainer_args, data_collator, train_dataset, val_dataset, tokenizer
        )
