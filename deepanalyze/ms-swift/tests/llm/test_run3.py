    ...
    def test_template_compare(self):
        ...
        for model_name, model_meta in MODEL_MAPPING.items():
            ...
            for group in model_meta.model_groups:
                model = group.models[0]
                if (
                    "awq" in model.ms_model_id.lower()
                    or "gptq" in model.ms_model_id.lower()
                ):
                    break
                ...
                try:
                    cmd = (
                        "CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python tests/llm/load_template.py "
                        f"--ms_model_id {model.ms_model_id} --template {template}"
                    )
                    if os.system(cmd) != 0:
                        raise RuntimeError()
                    ...
                    self.assertTrue(
                        np.allclose(
                            input_ids_new["input_ids"], input_ids_old["input_ids"]
                        )
                    )
                except Exception:
                    import traceback
