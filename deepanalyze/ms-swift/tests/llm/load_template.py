...
def load_and_tokenize(ms_model_id, template):
    ...
    try:
        ...
        model_ins, tokenizer = get_model_tokenizer(
            ms_model_id, load_model="mplug" in ms_model_id.lower()
        )
        ...
        if "audio" in template_ins.__class__.__name__.lower():
            ...
            output = EncodePreprocessor(template_ins)(
                load_ds("modelscope/DuReader_robust-QG")
            )
            input_ids = output[0].get("input_ids")
        if isinstance(output, Mapping):
            assert (
                output.get("input_ids") is not None
                or output.get("inputs_embeds") is not None
            )
        else:
            assert (
                output[0].get("input_ids") is not None
                or output[0].get("inputs_embeds") is not None
            )
        input_ids = to_list(input_ids)
        sent = ""
        try:
            if not isinstance(tokenizer, PreTrainedTokenizerBase) and hasattr(
                tokenizer, "tokenizer"
            ):
