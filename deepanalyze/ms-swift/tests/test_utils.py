...
TEST_MODEL_CHINESE_NAME = "Internal Test Model"
...
def get_case_model_info():
    ...
    for line in lines:
        ...
        case_info.add(
            test_file.replace("tests/", "").replace(".py", "").replace("/", ".")
        )

    return model_cases


def compare_arguments_nested(
    print_content, arg1, arg2, rtol=1.0e-3, atol=1.0e-8, ignore_unknown_type=True
):
    type1 = type(arg1)
    type2 = type(arg2)
    ...
    if arg1 is None:
        ...
        if not all(
            np.isclose(arg1, arg2, rtol=rtol, atol=atol, equal_nan=True).flatten()
        ):
            if print_content is not None:
                print(f"{print_content}")
            return False
    ...
    def _start(
        ...
        **kwargs,
        ...
        tmp_env["PYTHONPATH"] = ":".join(
            (tmp_env.get("PYTHONPATH", ""), script_dir)
        ).lstrip(":")
        # avoid distributed test hang
        tmp_env["NCCL_P2P_DISABLE"] = "1"
        script_params = "--save_all_ranks=%s --save_file=%s" % (
            save_all_ranks,
            tmp_res_file,
        )
    ...
    def start(
        ...
        **kwargs,
        ...
        if "dist_start_cmd" in kwargs:
            ...
            dist_start_cmd = (
                "%s -m torch.distributed.launch --nproc_per_node=%d "
