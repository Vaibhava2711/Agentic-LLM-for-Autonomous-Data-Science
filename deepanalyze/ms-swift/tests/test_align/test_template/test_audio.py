...
def _infer_model(pt_engine, system=None, messages=None, audios=None):
    ...
    resp = pt_engine.infer(
        [{"messages": messages, "audios": audios}], request_config=request_config
    )
    response = resp[0].choices[0].message.content
    messages += [{"role": "assistant", "content": response}]
...
def test_qwen2_audio():
    ...
    audios = [
        "https://qianwen-res.oss-cn-beijing.aliyuncs.com/Qwen2-Audio/audio/guess_age_gender.wav"
    ]
...
def test_gemma3n():
    ...
    messages = [
        {
            "role": "user",
            "content": "<audio>Transcribe this audio and complete the statement",
        }
    ]
    audios = [
        "https://qianwen-res.oss-cn-beijing.aliyuncs.com/Qwen2-Audio/audio/guess_age_gender.wav"
