...
def _infer_model(pt_engine, system=None, messages=None, videos=None, max_tokens=128):
    ...
    resp = pt_engine.infer(
        [{"messages": messages, "videos": videos}], request_config=request_config
    )
    response = resp[0].choices[0].message.content
...
def test_minicpmo():
    ...
    assert (
        ...
        == (
            ...
            "scene of a young reader immersed in their world of stories."
        )[: len(response)]
    )
...
def test_qwen2_5_omni():
    ...
    if USE_AUDIO_IN_VIDEO:
        ground_truth = (
            "Oh, that's a really cool drawing! It looks like a guitar. You've got the body "
