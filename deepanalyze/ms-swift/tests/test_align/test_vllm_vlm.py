...
def _infer_audio(
    model, use_chat_template: bool = True, max_model_len=8192, system=None
):
    engine = VllmEngine(
        model, max_model_len=max_model_len, limit_mm_per_prompt={"audio": 2}
    )
    ...
    resp_list = engine.infer(
        [InferRequest(messages=messages, audios=audios)],
        RequestConfig(temperature=0, max_tokens=64, repetition_penalty=1.0),
    )
...
def _infer_image(
    ...
    engine = VllmEngine(
        model, max_model_len=max_model_len, limit_mm_per_prompt={"image": 5, "video": 2}
    )
    if not use_chat_template:
        engine.default_template.use_chat_template = False
    ...
    resp_list = engine.infer(
        [InferRequest(messages=messages, images=images)],
        RequestConfig(temperature=0, max_tokens=64, repetition_penalty=1.0),
    )
    return resp_list[0].choices[0].message.content
...
def _infer_video(
    ...
    engine = VllmEngine(
        model, max_model_len=max_model_len, limit_mm_per_prompt=limit_mm_per_prompt
    )
    if not use_chat_template:
        engine.default_template.use_chat_template = False
    ...
    resp_list = engine.infer(
        [InferRequest(messages=messages, videos=videos)],
        RequestConfig(temperature=0, max_tokens=64, repetition_penalty=1.0),
    )
    return resp_list[0].choices[0].message.content
...
def test_deepseek_vl_v2():
    ...
    assert response == (
        "The image depicts a close-up of a adorable kitten with large, expressive eyes. The kitten has "
...
def test_internvl2():
    ...
    assert response == (
        "The image features a kitten with striking blue eyes and a mix of white and black fur. "
...
def test_qwen2_5_vl_video():
    ...
    assert response == (
        "A baby wearing sunglasses is sitting on a bed and reading a book. "
...
def test_qwen2_5_omni():
    ...
    response = _infer_video(
        "Qwen/Qwen2.5-Omni-7B", limit_mm_per_prompt=limit_mm_per_prompt
    )
